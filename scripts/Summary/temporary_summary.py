...
def save_checkpoint(output_path, content):
    with open(output_path, 'a', newline='', encoding='utf-8') as csvfile_out:
...
def main():
    ...
    with open(input_path, 'r', newline='', encoding='utf-8') as csvfile_in:
        ...
        for row in csv_reader:
            ...
            save_checkpoint(output_path, {"Content": main_content, "Summary and Q&A": f"Summary:\n{summary}"})
            for qna in qna_list:
                save_checkpoint(output_path, {"Content": main_content, "Summary and Q&A": qna})
